...
# ==================== CONFIG ====================
LOT_SIZES = {"NIFTY": 75, "BANKNIFTY": 15, "SENSEX": 10}
NIFTY_SPREAD = 700
SENSEX_SPREAD = 2200
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if data.get('data'):
                d = data['data'][0]
                return {
                    'value': float(d.get('last', 0) or 0),
                    'change': float(d.get('change', 0) or 0),
                    'pct': float(d.get('pChange', 0) or 0),
                    'source': 'NSE Live'
                }
    except Exception:
        _use_simulated = True
...
def calculate():
    ...
    atm = float(d["atm"])
    strategy = d["strategy"]
    put_dist = float(d["put_dist"])
    call_dist = float(d["call_dist"])
    lot_size = int(d["lot_size"])
    lots = int(d["lots"])
    premium_pe = float(d.get("premium_pe", 0))
    premium_ce = float(d.get("premium_ce", 0))

    qty = lot_size * lots
    sell_pe = round(atm - put_dist) if strategy in ("SELL_PE", "SELL_BOTH") else None
    sell_ce = round(atm + call_dist) if strategy in ("SELL_CE", "SELL_BOTH") else None

    p_pe = premium_pe if strategy in ("SELL_PE", "SELL_BOTH") else 0
    p_ce = premium_ce if strategy in ("SELL_CE", "SELL_BOTH") else 0
    total_credit = (p_pe + p_ce) * qty

    notes_map = {
        "SELL_PE": "Downside risk if market falls sharply.",
    ...
    return jsonify({
        ...
        "risk_note": notes_map.get(strategy, ""),
        "atm": atm,
        "strategy": strategy,
        "lots": lots,
        "lot_size": lot_size
...
def save_checklist():
    ...
    conn = get_db()
    conn.execute("""
        INSERT INTO checklists (symbol, checklist_type, all_checked, score, checklist_json, notes)
        VALUES (?,?,?,?,?,?)
...
def save_postmarket():
    ...
    mp = float(d.get("max_profit", 0))
    mnp = float(d.get("min_profit", 0))
    ml = float(d.get("max_loss", 0))
    if pnl <= -abs(ml):
        status = "LOSS HIT"
    elif pnl < 0:
        status = "SMALL LOSS"
    ...
    conn = get_db()
    conn.execute("""
        INSERT INTO post_market_analysis (trade_date, max_profit, min_profit, max_loss, daily_pnl, status, justification)
        VALUES (?,?,?,?,?,?,?)
...
def performance():
    ...
    conn.close()
    if not trades:
        return jsonify({"total": 0, "wins": 0, "losses": 0, "win_rate": 0, "total_pnl": 0, "avg": 0, "monthly": []})
    ...
    return jsonify({
        ...
        "avg": round(total_pnl / len(trades), 2),
